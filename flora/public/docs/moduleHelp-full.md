...
- На карте остаются только группы ≥10 точек с полностью одинаковыми координатами; остальные точки скрыты.
...
- Данные не записываются в Firestore / `points.json`; после загрузки сохраняются в IndexedDB браузера и восстанавливаются при следующем запуске. Повторная загрузка **добавляет** точки к уже сохранённым (слияние по ID источника). Загруженные точки участвуют в фильтрах карты и инструментах наравне с локальными.
...
- Перед загрузкой показывается оценка числа находок. Загрузка постраничная; карта обновляется пачками страниц; есть прогресс и отмена. Если наблюдений iNaturalist больше 10 000, загрузка выполняется **сериями** (по группам таксонов и при необходимости по годам). Для iNaturalist можно выбрать качество наблюдений: research grade, casual или оба.
