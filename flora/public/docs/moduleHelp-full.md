...
- **Контуры на карте** включает или выключает слой границ. Пока слой выключен, список субъектов недоступен.
- Блок **Отображение** задаёт прозрачность заливки, толщину и цвета контуров; настройки сохраняются локально.
- Список субъектов сгруппирован по федеральным округам. Чекбокс скрывает или показывает контур субъекта; клик по названию выбирает регион, подсвечивает его и приближает карту. Клик по контуру на карте тоже выбирает субъект.
- Кнопка фильтра **Только эти** оставляет на карте только точки внутри выбранного субъекта. Сброс — угловая кнопка фильтра.
