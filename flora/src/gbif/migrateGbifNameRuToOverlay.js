...
/**
 * Переносит name_ru из GBIF snapshot в overlay и очищает raw features.
 * @returns {Promise<{ collection: object, migrated: number, stripped: number }>}
 */
export async function migrateGbifNameRuToOverlay(collection) {
  ...
  for (const feature of collection.features) {
    if (feature?.properties?.source !== "gbif") {
      continue;
    }
  ...
  const features = collection.features.map((feature) => {
    if (feature?.properties?.source !== "gbif" || !feature.properties?.name_ru) {
      return feature;
    }
