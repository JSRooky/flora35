...
Модуль **Данные GBIF** загружает находки (occurrences) из открытого API GBIF напрямую в браузер и показывает их на **отдельном** слое карты (`gbif-locations`).

- Данные не записываются в `points.json`, `userpoints.json` и не попадают в фильтр источника «Проверенные / Пользовательские».
...
- **Только GBIF** скрывает локальные маркеры и тепловую карту, оставляя на карте слой GBIF; при выключении восстанавливается прежняя видимость.
