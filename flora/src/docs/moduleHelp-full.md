...
- Перед загрузкой показывается оценка числа находок. Загрузка постраничная; карта обновляется пачками страниц; есть прогресс и отмена. Если находок GBIF больше 10 000, загрузка идёт **сериями по годам** (и при необходимости по месяцам), плюс отдельные проходы по датасетам для записей без года. Если наблюдений iNaturalist больше 10 000, загрузка выполняется **сериями** (по группам таксонов и при необходимости по годам). Для iNaturalist можно выбрать качество наблюдений: research grade, casual или оба.
